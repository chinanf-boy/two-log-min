...
test('debug default  ', t => {
	...
	let m = s + ' two-log-min'
	t.is(
		l.start('log  default   starting').test,
		m
	);
	...
	t.is(
		l.stop('log  default   stopping', { ora: oraState, log: debug }),
		m
	);
});
...
test('debug default : no set  ', t => {
	...
	let m = s + ' two-log-min'
	t.is(
		l.start('log  default   starting', { ora: 'yellow' }).test,
		m
	);
...
test('debug use API, set debug formatter %h ', t => {
	...
	let debug = '';
	let m = s + ' two-log-min'
	let backLog = l.start('this is hex: %h', new Buffer('hello world'), { log: debug })
	let T = backLog.test
	t.is(
		T,
		m
